...
namespace Microsoft.EntityFrameworkCore.Specification.Tests
{
        ...
        where TFixture : NorthwindQueryRelationalFixture, new()
    {
        ...
        [Fact]
        public virtual void Last_with_order_by_does_not_issue_client_eval_warning()
        {
            using (var context = CreateContext())
            {
                var query1 = context.Customers.OrderBy(c => c.CustomerID).Last();
                Assert.NotNull(query1);

                var query2 = context.Customers.Where(c => c.CustomerID == "ALFKI" && c.Orders.OrderBy(o => o.OrderID).Last().OrderID > 1000).ToList();
                Assert.NotNull(query2);

                var query3 = context.Customers.OrderBy(c => c.CustomerID).LastOrDefault();
        ...
        [Fact]
        public virtual void LastOrDefault_with_order_by_issues_client_eval_warning_in_subquery()
        {
            using (var context = CreateContext())
            {
                Assert.Equal(CoreStrings.WarningAsErrorTemplate(
                    ...
                    Assert.Throws<InvalidOperationException>(
                        () => context.Customers.Where(c => c.CustomerID == "ALFKI" && c.Orders.OrderBy(o => o.OrderID).LastOrDefault().OrderID > 1000).ToList()).Message);
            }
        }
